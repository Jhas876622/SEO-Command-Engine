...
def _generate_fixes(export_dir, issues, site_name, use_ollama, max_fixes=20, max_redirects=30):
    title_fixes = []
    redirect_map = []
    df = None
    try:
        ...
        urls_to_fix = _unique(candidates)[:max_fixes]
    ...
    try:
        if df is None:
            df = load_export(Path(export_dir) / "internal_all.csv")
        redirect_map = generate_redirect_map(df, max_redirects=max_redirects)
    except Exception as e:
...
def _parser():
    ...
    ap.add_argument("--max-fixes", type=int, default=20, help="Maximum number of titles to rewrite (default: 20)")
    ap.add_argument("--max-redirects", type=int, default=30, help="Maximum number of 404 redirects to map (default: 30)")
    return ap
