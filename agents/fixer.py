...
# Ask Ollama using local HTTP REST API with subprocess fallback.
def _ollama_request(prompt, model, timeout=30):
    try:
    ...
    # Fallback to subprocess if HTTP port is closed or unavailable
    try:
...
# Ask Ollama for one title using bounded execution.
def _ollama_title(prompt, model):
    res = _ollama_request(prompt, model)
    return res.strip('"').strip("'")


# Ask Ollama for a batch response using bounded execution.
...
def generate_redirect_map(df, max_redirects=30):
    try:
        ...
        broken_urls = addresses[status.between(400, 499)].replace("", pd.NA).dropna().tolist()[:max_redirects]
